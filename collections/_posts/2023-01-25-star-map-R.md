...
Oh my! Can we do this with **R**? The answer is: Of course! In fact, [Kim
Fitter](https://kimnewzealand.github.io/) already worked on this two years earlier,
see his [Celestial
Maps](https://kimnewzealand.github.io/2019/02/21/celestial-maps/). So I decided
to put some work on this.

Since then, I also learnt that Benjamin and I have been working in parallel
on the same topic. He is preparing an **R** package named
...
same, and it is provided by the D3 plugin
...
Solution? I processed and fixed almost every file, including the files
corresponding to the Milky Way and the lines for Chinese constellations, manually,
...
[^1]:
    In fact, the download is performed via the
    [jsDelivr](https://www.jsdelivr.com/), which distributes files hosted on
...
result is an alternative longitude that we use to adjust the projection of our
Star Map. This provides the rotation observed in the **d3-celestial
plugin**.
...
The final result would have a spherical outline. That means that we would need
to perform a spherical cut. Did you know that [in r-spatial the Earth is no
...
plotting to specify the correct time zone. Since we already know the latitude and
...
Some online shops that create this kind of map (I won't post links) include
this script:
...
<img src="https://dieghernan.github.io/assets/img/blog/20230125_grat-1.webp" alt="plot of chunk 20230125_grat" width="100%" />

```r

# Cut to buffer; we don't flip this one (it is not an object of the space)
...
- We can also add a glowing effect to our stars and constellations (have a
  look at Dominic Royé's post [Firefly
...
calculations in JavaScript, and everything seems to be up and running.
...
Meeus J (1998). _Astronomical algorithms_, 2nd edition. Willmann-Bell,
