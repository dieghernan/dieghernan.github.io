...
subtitle: "A solution for black-and-white and academic maps"
tags:
  - r_bloggers
  - rstats
  - rspatial
...
**Updated 17 February 2020**: All these pieces of work are already available
in **cartography** >v.2.4.0 through the [`hatchedLayer`](http://riatelab.github.io/cartography/docs/reference/hatchedLayer.html)
and [`legendHatched`](https://riatelab.github.io/cartography/docs/reference/legendHatched.html)
functions. Just install it via `install.packages("cartography")`. A dedicated
blog post with examples is available at this [link](../202002_cartography1).
{: .alert .alert-info .p-3 .mx-2 .mb-3 }

In this post, I show how to produce different filling patterns that can be added
over your shapefiles with the **cartography** package.
...
As a complementary function, there is also `legendHatched`. The main parameters
are:

- `pos`, `title.txt`, `title.cex`, `values.cex`, `categ`, `cex` and `frame`:
  See `?cartography::legendTypo`.
...
- As in the case of the `hatchedLayer` function, different graphical
  parameters can be passed (`lty`, `lwd`, `pch`, `bg` on points).

Note that it is also possible to create solid legends by setting `col` and
`ptrn.bg` to the same color. Parameters respect the order of the `categ`
variable.
...
I hope that you find these functions useful. Enjoy, and happy mapping!
