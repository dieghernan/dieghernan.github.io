...
The only thing I was not completely comfortable with was that the post used the [old-fashioned](https://geocompr.robinlovelace.net/intro.html#the-history-of-r-spatial) **sp** package instead of my personal favorite, the **sf** package (the post was published in 2016; note that the author has also moved to the **sf** package since then). So I decided to explore further options with **sf**.

The approach is similar (using grids over a map and working with them) but using [`st_make_grid`](https://www.rdocumentation.org/packages/sf/versions/0.7-4/topics/st_make_grid). I also expanded it by grouping the grids and producing dots over the geometries. So basically I produced five variations of the map:
