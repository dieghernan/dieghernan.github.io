...
[CRAN](https://cran.r-project.org/package=resmush)! This is one tiny utility
package that allows to optimize (i.e. compress the size) of local and online
images using [reSmush.it](https://resmush.it/).

## What is reSmush.it?

reSmush.it is a **free online API** that provides image optimization, and it has
...
One of the main reasons I developed **resmush** is because I used to ship
[precomputed
vignettes](https://ropensci.org/blog/2019/12/08/precompute-vignettes/) with my
packages (see
[**tidyterra**](https://cran.r-project.org/web/packages/tidyterra/vignettes/welcome.html)
as an example). I found that the plots created by CRAN with the standard
configuration (i.e., not precomputed vignettes but built on CRAN itself) are not
very satisfying, and in some of the packages I developed (especially those
related to mapping), they don't do justice to the actual results when a user
runs them.

This approach has the drawback that it produces images of higher quality at the
expense of size. So, to avoid reaching [CRAN's 5Mb max size
...
development and SEO optimization. For example, I optimized all the images in
...
post](#altern), but in one line, reSmush.it API performs fast with minimal
configuration for a wide range of formats without an API key.
...
```{r local-ggplot2, message=FALSE, fig.cap="Original file"}
library(tidyterra)
library(ggplot2)
library(terra)
library(maptiles)
...
Cool, but the file has a size of `r pretty_size("cyl.png")`. So we can use
`resmush_file()` to reduce it, see:

```{r local-resmush, message=TRUE, fig.cap="Optimized file"}
library(resmush)
...
By default, `resmush_file()/resmush_dir()` do not overwrite the original file,
although this may be modified with the parameter `overwrite = TRUE`. Now the
resmushed file ("cyl_resmush.png") has a size of
`r pretty_size("cyl_resmush.png")`.

Let’s compare the results side-by-side:

::: figure
![Original online figure](cyl.png){alt="Original online figure"} ![Optimized
figure](cyl_resmush.png){alt="Optimized figure"}
...
We can check the reduction in size without reducing the dimensions of the image.
...
We can also optimize online files with `resmush_url()` and download them on
disk. In this example I present a feature of all the functions of **resmush**,
which is that they return an invisible data frame with a summary of the process.

```{r online, message=TRUE}
...
::: figure
![Original online
figure](https://dieghernan.github.io/assets/img/samples/sample_1.3mb.jpg){alt="Original online figure"}
![Optimized figure](sample_optimized.jpg){alt="Optimized online figure"}

<p class="caption">

Original picture (top) 1.3 Mb and optimized picture (bottom) 985 Kb (Compression
26.63%)

</p>
:::

## Other alternatives {#altern}

There are other alternatives for optimizing images for **R**, but first...

::: {.alert .alert-info .p-3 .mx-2 .mb-3}
<p><a href="https://yihui.org/">Yihui Xie</a>, one of the most prominent figures
in the <strong>R</strong> community, has recently been laid off from his
position at Posit PBC (formerly RStudio)
(<a href="https://yihui.org/en/2024/01/bye-rstudio/">more info</a>).</p>

<p>Yihui is the developer of <code>knitr</code>, <code>markdown</code>,
<code>blogdown</code>, and <code>bookdown</code>, among others, and he has been
one of the key contributors (if not the most) to reproducible research space
with <strong>R</strong> through his libraries.</p>

<p>If you have ever used and enjoyed his packages, consider sponsoring him.</p>
...
-   One of the many packages developed by Yihui Xie is
    ...
    -   `xfun::optipng()` compresses local files with OptiPNG (that needs to be
        installed locally).
...
are included in a specific PR before merging into your work.
