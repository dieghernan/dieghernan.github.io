...
been implemented on Wordpress, Drupal or Magento. Some of the features of
reSmush.it are:
...
related to mapping), they don't do justice to the actual results when a use**R**
runs them.

This approach has the drawback that it produces images of higher quality at a
...
Let's present an example of how a local file can be optimized. First we create a
large plot with **ggplot2**
...
altough this may be modified with the parameter `overwrite = TRUE` Now the
...
We can chech the reduction of size without reducing the dimensions of the image.
...
that is that they return an invisible data frame with a summary of the process.
