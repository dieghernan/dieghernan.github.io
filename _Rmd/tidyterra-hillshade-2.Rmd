...
# Using mapSpain
the_plain <- esp_get_prov(c("Madrid", "Castilla-La Mancha",
...
meteo_plot <- hill_plot +
  ...
  # Overlay the_plain
 geom_spatvector(data = r_plain, color = alpha("black", 0.7),
                 linewidth = .1) +
  ...
  theme(
    ...
    plot.subtitle = element_text(size = base_text_size * 0.8,
                                 hjust = 0.5),
    plot.caption = element_text(
...
base_plot <- hill_plot +
  ...
  scale_fill_gradientn(colors = mypal, na.value = NA,
                       limits = p_range)
