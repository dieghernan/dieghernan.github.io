...
The production of this map by @grossenbacher2016 has been a reference for years.
However, last developments on the **R** package ecosystem (`terra`, `sf` and
support of both classes on `ggplot2`, development of `ggnewscale`, etc.) can
make even easier the task of producing such type of maps.

In fact, @roye2022 recently wrote a very detailed post on creating shadow
effects on map reliefs. On this first post of the series I would replicate that
technique with a slight variation (e.g. not making use of `ggnewscale`) and I
would discuss a bit on the potential choice of a color palette for this kind of
maps.
...
First step is to get the altitude data. I use here the package `geodata`
[@grossenbacher2016] for simplicity, but you can use as well `elevatr` that is
much more complete. However `elevatr` produces the result as `RasterLayers`, so
you would need to convert the object to `SpatRaster` with `terra::rast()`.

```{r }

r_init <- elevation_30s("ROU", path = ".")
...
# Remove
r_init <- terra::spatSample(r_init, 50000, method = "regular", as.raster = TRUE)
...
[@Patterson_Jenny_2011]. For example, by convention green colors are associated
to low elevations while orange, browns and whites are associared to high
elevations on some of the most common elevation palettes (aka hypsometric
...
myload_fonts <- function(fontname, family, fontdir = tempdir()) {
  ...
  download.file(paste0("https://fonts.google.com/download?family=", fontname),
    fontzip,
    quiet = TRUE,
    mode = "wb"
  )
